...
# Argument parser
parser = argparse.ArgumentParser(description="Run substrate test with configurable input files.")
...
barplots_dir = os.path.join(args.output_dir, "substrates_barplots")
...
results_path = os.path.join(args.output_dir, "substrate_example_results.csv")
...
explainer = Explainer(
    model=model,
    algorithm=GNNExplainer(lr=0.005, epoch=700),
...
output_dir = os.path.join(args.output_dir, "substrates_barplots")
...
for idx, row in explain_df.iterrows():
    ...
    bars = plt.barh(row_sorted.index, row_sorted.values, color="blue")
